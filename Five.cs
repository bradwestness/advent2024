...
public sealed class Five : IAdventDay
{
    public string ExecutePartOne(string input)
    {
        var puzzleInput = PuzzleInput.ParseFrom(input);

        var total = 0;

        foreach (var manualUpdate in puzzleInput.ManualUpdates)
        {
            if (manualUpdate.IsOrderCorrect(puzzleInput.OrderingRules)
                && manualUpdate.MiddlePageNumber is int middlePageNumber
                && middlePageNumber != -1)
            {
        ...
        return $"Sum of middle page numbers of valid manual updates: {total}";
    }
    ...
    public string ExecutePartTwo(string input)
    {
        ...
        {
            if (!manualUpdate.IsOrderCorrect(puzzleInput.OrderingRules)
                && manualUpdate.CorrectPageOrder(puzzleInput.OrderingRules) is ManualUpdate corrected
                && corrected.MiddlePageNumber is int middlePageNumber
                && middlePageNumber != -1)
            {
                total += middlePageNumber;
            }
        ...
        return $"Sum of middle page numbers of corrected manual updates: {total}";
    }

    private sealed record PuzzleInput(IReadOnlyList<PageOrderingRule> OrderingRules, IReadOnlyList<ManualUpdate> ManualUpdates)
    {
        public static PuzzleInput ParseFrom(string input)
        {
            using var sr = new StringReader(input);

            var orderingRules = new List<PageOrderingRule>();
            var manualUpdates = new List<ManualUpdate>();
            ...
            while (sr.Peek() != -1)
            {
                ...
                else if (line.IndexOf('|') != -1)
                {
                    ...
                    if (tokens.Length == 2
                        ...
                        && int.TryParse(tokens[1], out var last))
                    {
                        orderingRules.Add(new PageOrderingRule(first, last));
                    }
                }
                else if (line.IndexOf(',') != -1)
                {
                    ...
                    {
                        manualUpdates.Add(new ManualUpdate(pageNumbers));
                    }
                }
            }

            return new PuzzleInput(orderingRules, manualUpdates);
        }
    };
    ...
    private sealed record ManualUpdate(IReadOnlyList<int> PageNumbers)
    {
        ...
        public bool IsOrderCorrect(IReadOnlyList<PageOrderingRule> orderingRules)
        {
            for (var i = 0; i < this.PageNumbers.Count; i++)
            {
                ...
                foreach (var (first, last) in orderingRules)
                {
                    if (pageNumber == first)
                    {
                        // ensure there are no pages before this one
        ...
        public static int IndexOf<T>(IList<T> list, T value)
        {
        ...
        public ManualUpdate CorrectPageOrder(IReadOnlyList<PageOrderingRule> orderingRules)
        {
            ...
            // correcting any invalid page orders
            // until there are no longer any pages that
            // violate any ordering rules
            while (hasInvalidPageOrder)
            {
                hasInvalidPageOrder = false;

                foreach (var (first, last) in orderingRules)
                {
                    for (var i = 0; i < corrected.Count; i++)
                    {
                        var pageNumber = corrected[i];

                        if (pageNumber == first)
                        {
                            var indexBefore = IndexOf(corrected, last);

                            if (indexBefore != -1 && indexBefore < i)
                            {
                                hasInvalidPageOrder = true;
                                corrected.RemoveAt(indexBefore);
                                ...
                                {
                                    corrected.Insert(i + 1, last);
                                }
                            }
                        }
                        else if (pageNumber == last)
                        {
                            var indexAfter = IndexOf(corrected, first);

                            if (indexAfter != -1 && indexAfter > i)
                            {
                                hasInvalidPageOrder = true;
                                corrected.RemoveAt(indexAfter);
                                corrected.Insert(i, first);
                            }
                        }
                    }
                }
            ...
            return new ManualUpdate(corrected);
        }
